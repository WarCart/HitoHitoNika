...
public class TankmanMorphModel<T extends LivingEntity> extends MorphModel<T> implements IHasArm {
    ...
    public TankmanMorphModel(boolean gomuAnimations) {
        super(-0.2F);
        this.texWidth = 64;
        this.texHeight = 64;
        ...
        this.head.setPos(0.0F, -4.4F, 0.0F);
    ...
    public void renderToBuffer(MatrixStack matrixStack, IVertexBuilder buffer, int packedLight, int packedOverlay, float red, float green, float blue, float alpha) {
        ...
        matrixStack.pushPose();
        matrixStack.translate(0.0D, -1.2D, 0.0D);
        ...
        matrixStack.popPose();
        matrixStack.pushPose();
        matrixStack.scale(4.4F, 3.0F, 9.0F);
        ...
        if (this.gomuAnimations && !(EntityStatsCapability.get(player).isBlackLeg() && CommonConfig.INSTANCE.isLegAbilities())) {
            ...
            matrixStack.pushPose();
            matrixStack.translate(1.6d - 0.5d * time, -0.6D + 0.05D * (double) time, 0.0D);
            matrixStack.scale(1.75F * 5.0F * time, 1.75F * 5.0F * time, 1.75F * 5.0F * time);
        } else {
            matrixStack.pushPose();
            matrixStack.translate(1.6D, -0.6D, 0.0D);
        ...
        matrixStack.pushPose();
        matrixStack.scale(1.75F, 1.75F, 1.75F);
        matrixStack.translate(-1.4D, -.6D, 0.0D);
        ...
        if (this.gomuAnimations && EntityStatsCapability.get(player).isBlackLeg() && CommonConfig.INSTANCE.isLegAbilities()) {
            ...
            if (ability != null && ability.isCharging()) {
                ...
                if (maxChargeTime == 0)
                    maxChargeTime = ability.getChargeTime();
                time += (1 - ability.getChargeTime() / maxChargeTime) * 0.8F;
            }
